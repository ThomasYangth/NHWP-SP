...
def run2D(model, edge, pbc = True):
    ...
    model = GenHN2D(tsp, ts0, tsm, f"2D{model}")
    sz1 = 200
    sz2 = 70
    T = 50

    pbc_sign = -1 if pbc else 1
    if edge.startswith("x"):
        plot_and_compare_2D_Edge(model, pbc_sign*sz1, sz2, T, force_evolve=False, Ns=sz1, edge=edge)
    else:
        plot_and_compare_2D_Edge(model, sz2, pbc_sign*sz1, T, force_evolve=False, Ns=sz1, edge=edge)
